...
class TaskRepository {
  ...
  Future<void> markMissed(String id) async {
    final task = await getTaskById(id);
    if (task != null) {
      await updateTask(task.copyWith(status: AppConstants.statusMissed));
    }
  }
...
class AnalyticsRepository {
  ...
  Future<void> updateDailyAnalytics(DateTime date, List<Task> tasks) async {
    ...
    final completedHigh = tasks
        ...
        .length;
    final totalHigh = tasks
        .where((t) => t.priority >= AppConstants.priorityHigh)
        .length;

    final completionScore = total == 0 ? 0.0 : (completed / total) * AppConstants.completionWeight;
    final priorityScore = totalHigh == 0 ? AppConstants.priorityWeight
