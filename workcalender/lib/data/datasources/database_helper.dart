...
class DatabaseHelper {
  ...
  Future<void> _createDB(Database db, int version) async {
    ...
    await db.execute('CREATE INDEX idx_tasks_date ON ${AppConstants.tableTasks}(date)');
    await db.execute('CREATE INDEX idx_tasks_status ON ${AppConstants.tableTasks}(status)');
    await db.execute('CREATE INDEX idx_analytics_date ON ${AppConstants.tableAnalytics}(date)');
  ...
  Future<int> updateTask(Map<String, dynamic> task) async {
    final db = await database;
    return await db.update(AppConstants.tableTasks, task,
        where: 'id = ?', whereArgs: [task['id']]);
  }

  Future<int> deleteTask(String id) async {
    final db = await database;
    return await db.delete(AppConstants.tableTasks, where: 'id = ?', whereArgs: [id]);
  }
  ...
  Future<List<Map<String, dynamic>>> getTasksByDate(DateTime date) async {
    ...
    return await db.query(AppConstants.tableTasks,
        where: 'date >= ? AND date <= ?',
  ...
  Future<List<Map<String, dynamic>>> getCategories() async {
    final db = await database;
    return await db.query(AppConstants.tableCategories,
        orderBy: 'isDefault DESC, name ASC');
  }

  Future<int> deleteCategory(String id) async {
    final db = await database;
    return await db.delete(AppConstants.tableCategories,
        where: 'id = ? AND isDefault = 0', whereArgs: [id]);
  }
  ...
  Future<List<Map<String, dynamic>>> getAnalyticsInRange(
      DateTime start, DateTime end) async {
    final db = await database;
    return await db.query(AppConstants.tableAnalytics,
        where: 'date >= ? AND date <= ?',
        whereArgs: [start.millisecondsSinceEpoch, end.millisecondsSinceEpoch],
        orderBy: 'date ASC');
  }
